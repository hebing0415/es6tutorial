...
实例的属性定义在this上，也可以在构造方式中使用this显示定义，方法都是定义在原型上（即定义在`class`上）。
...
上面代码中，`x`和`y`都是实例对象`point`自身的属性（因为定义在`this`变量上），`z`是class定义的属性，所以`hasOwnProperty()`方法返回`true`，而`toString()`是原型对象的方法，所以`hasOwnProperty()`方法返回`false`。这些都与 ES5 的行为保持一致。
